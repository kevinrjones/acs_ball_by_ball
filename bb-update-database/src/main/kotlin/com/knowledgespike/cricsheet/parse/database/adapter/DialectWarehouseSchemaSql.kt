...
internal object WarehouseSchemaSql {
    ...
    fun writeTo(writer: BufferedWriter, dialect: SqlDialect) {
        when (dialect) {
            SqlDialect.MARIADB -> writer.appendLine("USE cricsheet;")
            SqlDialect.POSTGRES -> {
                writer.appendLine("CREATE SCHEMA IF NOT EXISTS cricsheet;")
                writer.appendLine("SET search_path TO cricsheet;")
            }
